...
if (!isset($_SESSION['user_id'])) {
    ...
    $redirect = 'petDetails.php' . (!empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '');
...
if (!$pet) {
    header("Location: browsePet.php?error=Pet not found");
    exit;
}

// Check if pet is already adopted or has pet sitting
$isAlreadyAdopted = ($pet['PostType'] === 'Adopt' && $pet['Status'] === 'Adopted');
$isPetSitBooked = ($pet['PostType'] === 'Pet Sit' && $pet['Status'] === 'Sitting');
$isPetSitCompleted = ($pet['PostType'] === 'Pet Sit' && $pet['Status'] === 'Completed');
$isPetSitCancelled = ($pet['PostType'] === 'Pet Sit' && $pet['Status'] === 'Cancelled');
...
if (isset($_GET['from'])) {
    ...
    if ($from === 'map') {
        $backURL = 'petMap.php';
        $backText = 'Back to Map';
    } elseif ($from === 'profile') {
        if (isset($_GET['profile_user_id'])) {
            $backURL = 'profile.php?user_id=' . $_GET['profile_user_id'];
        } else {
            $backURL = 'profile.php';
        }
...
elseif (isset($_SERVER['HTTP_REFERER'])) {
    ...
    $fileMap = [
        ...
        'myPets.php' => 'myPets.php',
        'userDashboard.php' => 'userDashboard.php',
        'adminDashboard.php' => 'adminDashboard.php',
        'petMap.php' => 'petMap.php',
        'mapView.php' => 'petMap.php',
        'profile.php' => 'profile.php',
        'browsePet.php' => 'browsePet.php'
    ];

    if (isset($fileMap[$refererFile])) {
        $backURL = $fileMap[$refererFile];

        // Preserve query parameters from referer for specific pages
        if ($refererFile === 'profile.php') {
            $refererQuery = parse_url($referer, PHP_URL_QUERY);
            if ($refererQuery) {
                parse_str($refererQuery, $queryParams);
                if (isset($queryParams['user_id'])) {
                    $backURL .= '?user_id=' . $queryParams['user_id'];
                }
            }
        } elseif (in_array($refererFile, ['adminManagePets.php', 'adminBrowsePet.php', 'browsePet.php'])) {
            $refererQuery = parse_url($referer, PHP_URL_QUERY);
            if ($refererQuery) {
    ...
    $textMap = [
        ...
        'myPets.php' => 'Back to My Pets',
        'userDashboard.php' => 'Back to Dashboard',
        'adminDashboard.php' => 'Back to Dashboard',
        'petMap.php' => 'Back to Map',
        'mapView.php' => 'Back to Map',
        'profile.php' => 'Back to Profile',
        'browsePet.php' => 'Back to Browse'
    ];
...
// Check if user can request adoption or pet sitting
$canRequest = false;
$requestReason = '';
...
if ($userRole !== 'admin') {
    if ($pet['ApprovalStatus'] !== 'approved') {
        $canRequest = false;
        $requestReason = 'Pending Approval';
    } elseif ($isAlreadyAdopted) {
        $canRequest = false;
        $requestReason = 'Already Adopted';
    } elseif ($isPetSitBooked) {
        $canRequest = false;
        $requestReason = 'Pet Sitting Booked';
    } elseif ($isPetSitCompleted) {
        $canRequest = false;
        $requestReason = 'Pet Sitting Completed';
    } elseif ($isPetSitCancelled) {
        $canRequest = false;
        $requestReason = 'Pet Sitting Cancelled';
    } elseif ($pet['OwnerID'] == $currentUserId) {
        $canRequest = false;
        $requestReason = 'You are the Owner';
    } else {
        $canRequest = true;
    }
}
...
<head>
    <meta charset="UTF-8">
    <title><?php echo htmlspecialchars($pet['Name']); ?> - FurCare</title>
    ...
    <style>
        ...
        /* ===== Navbar ===== */
        .navbar {
            background: linear-gradient(90deg, #6DBE81, #FFB6C1);
            color: #fff;
            padding: 15px 25px;
            display: flex;
...
<body>
    ...
    <div class="navbar">
        ...
        <img src="<?php echo !empty($currentUserPicture) ? htmlspecialchars($currentUserPicture) : 'uploads/profile_icon.png'; ?>"
            alt="Profile"
            class="profile-icon">
    </div>
    ...
    <div class="main-content">
        <div class="pet-container">
            ...
            <div class="page-header">
                <h1>Pet Details</h1>
                <p>View detailed information about <?php echo htmlspecialchars($pet['Name']); ?></p>
            </div>
            ...
            <div class="details-grid">
                ...
                <div class="info-card">
                    ...
                    <?php if ($userRole === 'admin'): ?>
                        <div class="admin-info">
                            ...
                            <div class="admin-actions">
                                <a href="editPet.php?id=<?php echo $pet['PetID']; ?>" class="btn" style="background: linear-gradient(135deg, #9CA3AF, #6B7280); color: white; padding: 6px 12px; text-decoration: none; border-radius: 6px; font-size: 0.85rem; display: inline-flex; align-items: center; gap: 5px;">
                    ...
                    <div class="owner-info">
                        <h3><i class="fas fa-user"></i> Contact Owner</h3>
                    ...
                    <div class="action-buttons">
                        <?php if ($canRequest && $pet['ApprovalStatus'] === 'approved'): ?>
                            <!-- User can request -->
                            <?php if ($pet['PostType'] === 'Adopt'): ?>
                                <a href="adopt.php?pet_id=<?php echo $pet['PetID']; ?>&from=details" class="btn primary">
                                    <i class="fas fa-heart"></i> Adopt Me
                                </a>
                            <?php elseif ($pet['PostType'] === 'Pet Sit'): ?>
                                <a href="petSit.php?pet_id=<?php echo $pet['PetID']; ?>&from=details" class="btn primary">
                                    <i class="fas fa-home"></i> Request Pet Sitting
                                </a>
                            <?php endif; ?>
                        <?php else: ?>
                            <!-- User cannot request -->
                            <button class="btn primary" disabled>
                                ...
                                <?php echo $pet['PostType'] === 'Adopt' ? 'Adopt Me' : 'Request Pet Sitting'; ?>
                            </button>
                        <?php endif; ?>

                        <button onclick="sharePet()" class="btn secondary">
    ...
    <script>
        ...
        function sharePet() {
            const shareData = {
                title: '<?php echo htmlspecialchars($pet['Name']); ?> - FurCare',
                text: 'Check out this adorable pet on FurCare!',
