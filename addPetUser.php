...
// CHECK PHONE NUMBER - DIRECT QUERY
$phone_result = $conn->query("SELECT Phone FROM user WHERE UserID = $userID");
$phone_data = $phone_result->fetch_assoc();

if (empty($phone_data['Phone'])) {
    header("Location: userProfile.php?error=Please add your phone number first before posting pets");
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Calculate age properly from years and months - DIRECT QUERY VERSION
    $years = intval($_POST['years'] ?? 0);
    $months = intval($_POST['months'] ?? 0);

    // Ensure months is between 0-11
    $months = max(0, min(11, $months));
    $age = round($years + ($months / 12), 2); // Calculate properly in PHP
    ...
    $approvalStatus = 'pending'; // User posts always pending approval
    $approvedAt = "NULL";
    $approvedBy = "NULL";
    ...
    if (!isset($errorMsg)) {
        ...
        if ($conn->query($sql)) {
            header("Location: userDashboard.php?added=1&pending=1");
            exit;
        } else {
...
<head>
    <meta charset="utf-8" />
    <title>Post New Pet — FurCare</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="addPetUser.php" class="active">➕ Post a Pet</a>
    ...
    <div class="main-content">
        <div class="container">
            <h1 class="title">Post a New Pet</h1>
            <p class="subtitle">Share your pet's information to find them a loving home or pet sitter</p>
        </div>
            ...
            <div class="form-container">
                <div class="left-col">
                    ...
                    <div class="form-actions">
                        <a href="userDashboard.php" class="btn cancel">Cancel</a>
                        <button type="submit" class="btn primary">Post Pet</button>
                    </div>
                </div>
    ...
    <script>
        ...
        // Set min dates untuk start dan end date
        ...
        // Calculate duration and total price for pet sitting
        ...
        // Validate form sebelum submission
        ...
        // Force min dates setiap 10 saat
        setInterval(setMinDates, 10000);
    </script>
</body>
