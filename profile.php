...
// Dapatkan user ID dari URL atau default kepada current user
...
// Jika profile_user_id tidak valid, redirect ke halaman sendiri
if (!$profile_user_id) {
    $profile_user_id = $current_user_id;
}

// Ambil data current user untuk navbar
$current_user_stmt = $conn->prepare("SELECT ProfilePicture FROM user WHERE UserID = ?");
$current_user_stmt->bind_param("i", $current_user_id);
$current_user_stmt->execute();
$current_user_result = $current_user_stmt->get_result();
$current_user = $current_user_result->fetch_assoc();

// Dapatkan user data dengan trust level, rating purata, dan jumlah reviews
$user_sql = "SELECT u.*, 
                    ...
                    -- RATING PURATA DARIPADA REVIEWS - BARU
                    (SELECT COALESCE(AVG(Rating), 0) 
                     FROM review 
                     WHERE SitterID = u.UserID) as average_rating,
                    
                    -- BILANGAN REVIEWS - BARU
                    (SELECT COUNT(*) 
                     FROM review 
...
// Dapatkan reviews TENTANG user ini (orang lain review dia) - FIXED QUERY
...
if (isset($current_user['ProfilePicture']) && !empty($current_user['ProfilePicture'])) {
    $currentUserPicture = $current_user['ProfilePicture'];
}
...
<head>
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($user_data['Name']) ?> - Profile • FurCare</title>
...
<body>
    ...
    <div class="navbar">
        ...
        <img src="<?php echo $currentUserPicture; ?>"
            alt="Profile"
            class="profile-icon">
    </div>
    ...
    <div class="profile-container">
        ...
        <div class="profile-header">
            ...
            <div class="profile-info">
                ...
                <!-- Transaction Statistics (Tanpa Trust Score Percentage) -->
                ...
                <div class="pets-grid">
                    <?php while ($pet = $pets_result->fetch_assoc()): ?>
                        <div class="pet-card">
                            ...
                            <a href="petDetails.php?pet_id=<?= $pet['PetID'] ?>&from=profile&profile_user_id=<?= $profile_user_id ?>"
    ...
    <!-- JavaScript for Sidebar and Show More Reviews -->
    <script>
        function toggleSidebar() {
            // Function to toggle sidebar (you can add sidebar functionality here)
            window.location.href = "profile.php?user_id=<?php echo $current_user_id; ?>";
        }
